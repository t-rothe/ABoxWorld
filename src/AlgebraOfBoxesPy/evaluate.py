...
import sys
sys.path.append("S:\\Sync\\University\\2023_MRP_2\\MRP2_WorkDir\\ABoxWorld\\AlgebraOfBoxesPy")
import utils
#importlib.import_module("./AlgebraOfBoxutils")
...
def R_tensor(W, P, Q):     # W is a 32xn matrix, P and Q are 2x2x2x2 tensors
    P = utils.tensor_to_matrix(P)
    Q = utils.tensor_to_matrix(Q)
    T1 = torch.tensordot(A(W), P, dims=1)  # green term
    T2 = torch.transpose(torch.tensordot(B(W), Q, dims=1), 4, 3)  # blue term
    T3 = (T1*T2).repeat(2,2,1,1,1,1,1)
    T4 = T3 * C(W) * D(W)  # the big bracket
...
def h_flat(R):  # R is a 2x2x2x2xn tensor
    ...
    return torch.tensordot( utils.CHSH_flat, R, dims=1 )  # scalar product of CHSH and each column of R


def h_prime_flat(R):  # R is a 2x2x2x2xn tensor
    R = torch.reshape(R, (16, -1))
    return torch.tensordot( utils.CHSH_prime_flat, R, dims=1 )  # scalar product of CHSH and each column of R
...
def phi_power(W, P, N):
    ...
    for k in range(N-2):
        for alpha in range(nb_columns):
            Q[k+3,:,:,:,:,alpha] = R(W, utils.tensor_to_matrix(Q[k+2,:,:,:,:,alpha]), P)[:,:,:,:,alpha]
...
def box_power_recursive(W, P, N):
    ...
    for alpha in range(nb_columns):
        Q_prime[:,:,:,:,alpha] = R(W, utils.tensor_to_matrix(Q[:,:,:,:,alpha]), P)[:,:,:,:,alpha]
    return Q_prime
